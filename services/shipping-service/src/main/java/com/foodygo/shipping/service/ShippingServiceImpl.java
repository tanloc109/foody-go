...
import com.foodygo.shipping.exception.AlreadyExistsException;
...
import com.foodygo.shipping.service.kafka.ShippingProducer;
...
public class ShippingServiceImpl implements ShippingService {
    ...
    public ShippingCreationResponse createShipping(ShippingCreationRequest request) {
        if (shippingRepository.findByOrderId(request.orderId()).isPresent()) {
            throw new AlreadyExistsException("Order already exists a shipping");
        }
        ...
        notfifyShipperOfNewShipping(ShippingMapper.INSTANCE.toCreateDTO(shipping));
    ...
    private void notfifyShipperOfNewShipping(ShippingCreationResponse response) {
        shippingProducer.sendMessage("Newly created shipping: " + response.toString());
    }
